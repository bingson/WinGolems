...
; TEXT FUNCTIONS _______________________________________________________________
  ...
  +!sc029::              Clip("`````` " Clip() " ``````")                       ;ManipulateText: enclose selected text with ``` ```
  +!sc027::              send {{}                                               ;ManipulateText: curly braces open 
  ^!sc027::              send {}}                                               ;ManipulateText: curly braces close
...
; Applications  _____________________________________
  ...
  SetTitleMatchMode, 2
  #If WinActive("ahk_group FileListers") and GC("T_adv_opt",0)                  ; applies to ListView windows like file explorer and save as dialogue boxes
...
; FUNCTION BOX _________________________________________________________________
  ...
  ; The comma operator is usually faster than writing separate expressions, especially when assigning one variable to another (e.g. x:=y, a:=b). https://bit.ly/38G14mF
  ; Performance continues to improve as more and more expressions are combined into a single expression; for example, it may be 35% faster to combine five or ten simple expressions into a single expression.
  ...
  +#b:: Files   := { "f" : A_ScriptDir "\golems\_functions.ahk"                 ;FunctionBox: edit file
                   ...
                   , "i" : """" config_path """" }
      , FunctionBox("EditFile", Files, C.lblue)                                 ; no return statement required if I use the line continuation symbol = ","
