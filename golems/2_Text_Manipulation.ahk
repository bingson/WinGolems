; CONVENIENCE/TEXT MANIPULATION ________________________________________________
  <!capslock::        SI("{del}")                                               ;Convenience: del                                                                                               
  >!capslock::        SI("{del 5}")                                             ;Convenience: del 5 spaces                                                                                              
  ...
  <+^Backspace::     Sendinput {BackSpace 5}                                    ;Convenience: Send {BackSpace 5}
  <+^Space::         FillChar(5, " ", 0)                                        ;Convenience: Send {Space 5}     
  ...
  +!sc029::          Clip("`````` " Clip() " ``````")                           ;Convenience: enclose selected text with ``` ```
  #!sc028::          s("{blind}"), ReplaceAwithB("""", "")                      ;Convenience replace "_" with " " in selected text
  #!sc00C::          s("{blind}"), ReplaceAwithB("-", " ")                      ;Convenience replace "_" with " " in selected text
  ...
  #!sc033::          s("{blind}"), ReplaceAwithB(",", " ")                      ;Convenience replace "_" with " " in selected text
  ...
  #If !WinActive("ahk_exe Code.exe")                                            ; conditional: ignore this shortcut if vs code active            
  ^!down::              SelectLine(), txt := clip(), s("{right}{enter}"), clip(txt)         ;Convenience: duplicate current line
  #IF             
...
; TEXT NAVIGATION  _____________________________________________________________
    ...
    <+>+k::            Sendinput {up 12}                                        ;Selection: extend selection up    4 rows
    <+>+j::            Sendinput {down 12}                                      ;Selection: extend selection down  4 rows
  ...
  pgup & l::              Sendinput {Right 5}                                        ;Navigation: extend selection Right 5 character
  pgup & h::              Sendinput {Left 5}                                         ;Navigation: extend selection Left  5 character
  pgup & j::              Sendinput {down 5}                                         ;Navigation: extend selection Right 5 character
  pgup & k::              Sendinput {up 5}                                           ;Navigation: extend selection Left  5 character
  #IF
...
; TEXT SELECTION _______________________________________________________________
    ...
    #IF GetKeyState("lwin", "P") and GetKeyState("lalt", "P")
    lalt & h::         Sendinput +^{left 4}                                     ;Selection: extend selection up    4 rows
    lalt & l::         Sendinput +^{right 4}                                    ;Selection: extend selection down  4 rows
    #IF
    ...
    >+!l::             Sendinput +{Right 8}                                     ;Selection: extend selection Right 8 characters
    >+!h::             Sendinput +{Left 8}                                      ;Selection: extend selection Left  8 characters
    <+>+l::            Sendinput +{Right 5}                                     ;Selection: extend selection Right 5 characters
    <+>+h::            Sendinput +{Left 5}                                      ;Selection: extend selection Left  5 characters
    #IF IsCMODE() ; capslock- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 
    rshift & j::       Sendinput +{down 10}                                        ;Navigation: extend selection Right 2 character
    rshift & k::       Sendinput +{up 10}                                          ;Navigation: extend selection Left  2 character
    #IF
  
; MOUNCTION _______________________________________________________________
    ...
    *#d::             CursorRecall(substr(A_ThisHotkey,0), 1,,0)                ;MouseFn: return to saved mouse position and click (left click if no saved position found) chng 4th parameter to 1 stay at saved clicked position
    ...
    :X:+p~win::       SaveMousPos("p",1)                                        ;MouseFn: Left click and save mouse position
                      ...
                      DC("MPos_" CurrentDesktopNum() "_" substr(A_ThisHotkey,0))
