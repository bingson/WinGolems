...
;{-----------------------------------------------
 ...
 Set_HideFold		= 1
 Set_TextOut		= 1	; Set to 1 to automatically create text file output of Help Dialog
 Set_FindPos		= 1
...
;{-----------------------------------------------
	gosub, Refresh             ; bing: added to create HotKey Help - Dialog.txt
	ExitApp					   ; bing: added to create HotKey Help - Dialog.txt
	OnExit, SaveSettings  
 ;
;}-----------------------------------------------
; END OF AUTO-EXECUTE
		...
		if (Help[File,"Count"] > 0 and Help[File,"Type"] = "AHK")
		{
			...
			for Hot, Hot_Index in Help[File,"Hot"]
				for Hot_Index2, Info in Help[File,"Hot",Hot]
					if (Hot_Index2 != "Count")
						Display_Section .= A_space Format_Line(Hot,Info,Pos_Info) "`r`n"
